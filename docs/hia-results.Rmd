...
We measured existing green space in the City of Denver by the normalized difference vegetation index (NDVI) measured by the Landsat-8 satellite at a spatial resolution of 30 square meters. To define native-plants greenness, we measured the NDVI of areas in Denver that are mostly native, including the Denver Botanic Gardens and a few other locations on several cloud-free spring and summer days in 2021. Over the course of these days in these areas, the NDVI values tended to range between 0.4 and 0.5 (details [here](https://michaeldgarber.github.io/green-space-denver/ndvi-of-places-tracts.html)). The NDVI of native vegetation may vary by ecosystem conditions and level of maintenance. To accommodate this uncertainty and assess the sensitivity of results to the NDVI-based definition of native plants, we conducted analyses defining the NDVI of native plants at both 0.4 and 0.5.
...
hia_all_overall %>% 
  ...
  knitr::kable(
    ...
    format.args = list(big.mark = ","),
    digits = c(0, 0, 1, 0, 0, 0, 0, 0,0)) #set max digits (after decimal, I guess)
```
