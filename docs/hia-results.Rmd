...
This document presents initial results for an assessment of the potential health impact of adding more vegetated landscape to the City of Denver. We aim to estimate the number of deaths that would be prevented by adding more green space, specifically native plants, to the City of Denver. We estimate the number of deaths prevented corresponding to four categories of greening scenarios, which were informed by conversations with local stakeholders. The scenarios are:
...
hia_all_overall %>% 
  ...
  dplyr::arrange(scenario_sort_order) %>% 
  dplyr::select(scenario, scenario_sub, starts_with("pop_affected"), starts_with("attrib_d")) %>% 
  knitr::kable(
    col.names = c(
      "Scenario", "Sub-scenario", 
      "Pop. affected, est.", "Pop. affected, 95% LL", "Pop. affected, 95% UL",
      "Attrib. deaths, est.", "Attrib. deaths, 95% LL", "Attrib. deaths, 95% UL"
      ),
    digits = 0) #set max digits (after decimal, I guess)
```

# Next steps (as of April 18, 2022)
