...
1.  **Homogenous native-plants greening:** Under the first set of scenarios, we consider adding native plants homogenously across all block groups, without respect to the specific location. Specifically, we consider two scenarios:
    ...
    -   Set 30% of the block group's area to be as green as native plants, corresponding with a goal set by some scientists and advocates to protect 30% of lands and oceans by 2030[@dinerstein2019a];
...
hia_all_overall %>% 
  bind_rows(hia_all_equity_cat) %>% 
  left_join(hia_all_overall_s, by = c("scenario", "scenario_sub", "ndvi_native_threshold")) %>% 
  left_join(hia_all_equity_cat_s, by = c("scenario", "scenario_sub", "ndvi_native_threshold", "equity_bg_cdphe_tertile_den")) %>%   arrange(scenario_sort_order) %>% 
  ...
  knitr::kable(
    caption = "Population affected, cumulative area of the treatment, and the cumulative area of the 500 m residential buffer surrounding the treatment under each scenario",
    booktabs = TRUE,
    col.names = c(
...
hia_all_overall %>% 
  ...
  knitr::kable(
     booktabs = TRUE,
        caption = "Under each scenario, the estimated number of deaths prevented and the number of deaths prevented per 100,000 people affected",
    col.names = c(
      "Scenario", "Sub-scenario", 
      "NDVI, native",
