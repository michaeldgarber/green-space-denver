...
We measured existing green space in the City of Denver by the normalized difference vegetation index (NDVI) measured by the Landsat-8 satellite at a spatial resolution of 30 square meters. To define native-plants greenness, we measured the NDVI of several places in Denver (places listed [here](https://michaeldgarber.github.io/green-space-denver/ndvi-of-places-tracts.html#22_Load_places_of_interest))comprised of various levels of native and adapted plants during cloud-free summer (May, June, July, August) days over five years (2016-2021). Over these days in these areas, the [median NDVI](https://michaeldgarber.github.io/green-space-denver/ndvi-of-places-tracts.html#25_Simple_summary_(median)_of_NDVI_for_these_places) was as low as 0.3228098 at the Denver Botanic Gardens Green Roof, which is a mix of native and other plant types and exists in an urban environment and as high as 0.704 in City Park, which is 0% native and consists of more watered turf. A plot in Denver Botanic Gardens that is 100% native plants had a median NDVI of 0.49. 
...
For each definition of native-plants NDVI, We excluded census blocks whose baseline NDVI was above the threshold. This map visualizes the NDVI threshold at 0.4.
...
And this one defines the native-plants threshold at 0.5.
```{r,  eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE}
library(RColorBrewer)
setwd(here("data-processed"))
load("den_co_bg_ndvi_alt_all_nogeo.RData")
