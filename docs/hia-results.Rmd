...
We measured existing green space in the City of Denver by the normalized difference vegetation index (NDVI) measured by the Landsat-8 satellite at a spatial resolution of 30 square meters. To define native-plants greenness, we measured the NDVI of several places in Denver (places described in more detail [here](https://michaeldgarber.github.io/green-space-denver/ndvi-of-places-tracts.html#22_Load_places_of_interest))comprised of various levels of native and adapted plants during cloud-free summer (May, June, July, August) days over five years (2016-2021). Over these days in these areas, the mean NDVI was as low as 0.35 at the Denver Botanic Gardens Green Roof, which is a mix of native and other plant types and exists in an urban environment and as high as 0.67 in City Park, which is 0% native and consists of more watered turf. A plot in Denver Botanic Gardens that is 100% native plants had a mean NDVI of 0.54 over these days.
...
To acknowledge the lack of precision in these NDVI estimates and the range of NDVI levels that may correspond to a native-plants intervention, we defined NDVI using 0.49 (Denver Botanic Gardens 100% native plot) as the primary analysis and 0.322 (Denver Botanic Gardens Green Roof) as an alternative value.
...
For each definition of native-plants NDVI, We excluded census blocks whose baseline NDVI was above the threshold. This map visualizes the cutoff if the NDVI threshold is the lower of the two values, that of the Denver Botanic Gardens Green Roof.
...
mv_bg_below_native_threshold_04 =den_co_bg_ndvi_alt_all_nogeo %>% 
...
mv_bg_below_native_threshold_04@map %>% 
  addFullscreenControl()
```

And this one defines the native-plants threshold at a higher value, that of Denver Botanic Gardens Native Plot.
```{r,  eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE}
library(RColorBrewer)
setwd(here("data-processed"))
load("den_co_bg_ndvi_alt_all_nogeo.RData")
load("lookup_den_co_bg_no_wtr_geo.RData")
ndvi_below_thresh_pal = RColorBrewer::brewer.pal(3, "RdYlGn")[c(1,3)]  %>% rev()
mv_bg_below_native_threshold_05 =den_co_bg_ndvi_alt_all_nogeo %>% 
...
mv_bg_below_native_threshold_05@map %>% 
  addFullscreenControl()
```
...
hia_all_overall %>% 
  ...
  knitr::kable(
    ...
    format.args = list(big.mark = ","),
    digits = c(0, 0, 2, 0, 0, 0, 0, 0,0)) #set max digits (after decimal, I guess)
```
