...
mv_den_metro_bbox = den_metro_bbox %>% mapview(layer.name = "Study area")
mv_den_metro_bbox
```
...
```{r loadtestareas, eval=TRUE, echo=TRUE, warning=FALSE, message=FALSE, cache = TRUE}
setwd(here("data-processed"))
...
mv_den_metro_bbox = den_metro_bbox %>% mapview(layer.name = "Study area")
mv_den_metro_bbox+ mv_evergreen_east + mv_indian_tree_golf + mv_city_park
```
...
Hmm, if we look closely, we can see that there are some very low NDVI values for some of these places even in the summer, which is not plausible. Those low values must indicate obstruction by clouds. So I'm going to exclude those dates.
