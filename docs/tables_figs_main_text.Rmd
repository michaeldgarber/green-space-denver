...
# Tables option 1: (stratify by native-plants NDVI definition)
...
load("hia_all_by_ndvi_equity.RData")
load("hia_all_by_ndvi_equity_s.RData")
...
hia_all_by_ndvi_equity_est_boot = hia_all_by_ndvi_equity %>% 
  left_join(hia_all_by_ndvi_equity_s, by = c("scenario", "scenario_sub", "ndvi_native_threshold", "equity_bg_cdphe_tertile_den")) 
hia_all_by_ndvi_est_boot %>% 
  bind_rows(hia_all_by_ndvi_equity_est_boot) %>% 
  ...
  filter(ndvi_native_threshold < .4 | ndvi_native_threshold > .5) %>% #5/25 to keep just 2 results
  dplyr::select(
    scenario, 
...
hia_all_by_ndvi_est_boot %>% 
  ...
  knitr::kable(
    caption = "For each scenario and native-plants NDVI definition, the change in NDVI from baseline to the alternative value, the estimated number of deaths prevented, and the estimated number of deaths prevented per 100,000 population.",
    booktabs = TRUE,
    col.names = c(
      "Scenario", 
      "NDVI, native",
...
# Tables option 2: (report summary results summarized over plausible native-plants NDVI definitions)
