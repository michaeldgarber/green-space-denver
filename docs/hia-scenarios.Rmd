...
From this point, we followed the same framework as we've done for other projects. The baseline NDVI of the sampled parcels and that of their corresponding 500m buffers is visualized below.
...
To estimate the number of premature deaths prevented in each age group and area, we will multiply the population-attributable fraction by the baseline mortality rate by the population size. We will analyses to adults aged 30 and above (subject to change) following the age range of many of the cohort studies reviewed,[@rojas-rueda2019] and will exclude northeastern census block groups near the airport (subject to change; maybe not for each scenario).
